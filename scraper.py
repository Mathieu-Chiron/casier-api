...
def _build_entry(raw: dict) -> dict:
    ...
    return {
        ...
        "statut":      _statut(affaire),
        "source":      "casier-politique.fr",
        "url":         SOURCE_URL,
    }


async def _scrape(name: str) -> list:
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            print(f"[SCRAPER] {nb_pages} pages — recherche : {name}")
                ...
                try:
                    ...
                    for raw in raw_entries:
                        ...
                        if entry["affaire"]:
                            ...
                            print(f"[SCRAPER] page {num_page} : {entry['affaire'][:60]}")

                except Exception as e:
                    print(f"[SCRAPER] Erreur page {num_page}: {e}")
                    continue
...
async def get_condamnations(name: str) -> dict:
    try:
        condamnations = await asyncio.wait_for(_scrape(name), timeout=120)
        return {
